...
function addLiquidityETH(
    ...
    {
        require(path[0] == WETH, 'PalindromeV2Router: INVALID_PATH');
        amounts = ExchangeLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PalindromeV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
...
function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            IERC20(path[path.length - 1]).balanceOf(to) - (balanceBefore) >= amountOutMin,
            'PalindromeV2Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
